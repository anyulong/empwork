...
//    Predicate p ;
	  ...
	  //测试doubleUnary
	  double d1 =doubleUnary(x -> x, 10, 100, 1000);
	  System.out.println(d1);
...
//  }
  ...
  //用DoubleUnaryOperator替换
  public static double doubleUnary(DoubleUnaryOperator func, double x1, double x2, int numSlices) {
	    if (numSlices < 1) numSlices = 1;
	    double delta = (x2 - x1) / numSlices;
	    double start = x1 + delta / 2.0;
	    double sum = 0.0;
